export default function (playerInstance, options)
{
    playerInstance.setupThumbnailPreviewVtt = () =>
    {
        playerInstance.sendRequest(
            ...
            function ()
            {
                ...
                const webVttParser = new (window as any).WebVTTParser();
    ...
    {
        const progressContainer = document.getElementById(playerInstance.videoPlayerId + '_ww_controls_progress_container');
        const previewContainer  = document.createElement('div');

        previewContainer.id             = playerInstance.videoPlayerId + '_ww_timeline_preview_container';
        previewContainer.className      = 'ww_timeline_preview_container';
        ...
        previewContainerShadow.id             = playerInstance.videoPlayerId + '_ww_timeline_preview_container_shadow';
        previewContainerShadow.className      = 'ww_timeline_preview_container_shadow';
    ...
    {
        const timelinePreviewTag    = document.getElementById(playerInstance.videoPlayerId + '_ww_timeline_preview_container');
        const timelinePreviewShadow = document.getElementById(playerInstance.videoPlayerId + '_ww_timeline_preview_container_shadow');
        const progressContainer     = document.getElementById(playerInstance.videoPlayerId + '_ww_controls_progress_container');
    ...
    playerInstance.setupThumbnailPreview = () =>
    {
        ...
        document.getElementById(playerInstance.videoPlayerId + '_ww_controls_progress_container')
            .addEventListener(eventOn, playerInstance.drawTimelinePreview.bind(playerInstance), false);
        document.getElementById(playerInstance.videoPlayerId + '_ww_controls_progress_container')
            .addEventListener(eventOff, function (event: MouseEvent)
            {
                const progress = document.getElementById(playerInstance.videoPlayerId + '_ww_controls_progress_container');
                if (typeof event.clientX !== 'undefined' && progress.contains(document.elementFromPoint(event.clientX, event.clientY)))
                {
                    //False positive (Chrome bug when fast click causes leave event)
                    return;
                }
                document.getElementById(playerInstance.videoPlayerId + '_ww_timeline_preview_container').style.display        = 'none';
                document.getElementById(playerInstance.videoPlayerId + '_ww_timeline_preview_container_shadow').style.display = 'none';
            }, false);
        playerInstance.generateTimelinePreviewTags();
