...
export default function (playerInstance, options)
{
  ...
  playerInstance.renderLinearAd = (adListId, backupTheVideoTime) =>
  {
    ...
    const playVideoPlayer = adListId =>
    {
      playerInstance.switchPlayerToVpaidMode = adListId =>
      {
        ...
        const vpaidIframe: string         = playerInstance.videoPlayerId + '_' + adListId + '_fluid_vpaid_iframe';
  ...
  playerInstance.addAdCountdown = () =>
  {
    ...
    // Create element
    const adCountdown        = playerInstance.pad(Number(playerInstance.currentVideoDuration / 60)) + ':' + playerInstance.pad(Number(playerInstance.currentVideoDuration % 60));
  ...
  playerInstance.decreaseAdCountdown = function decreaseAdCountdown()
  {
    ...
    if (btn)
    {
      btn.innerHTML = '<span class=\'ad_timer_prefix\'>Ad - </span> ' + playerInstance.pad(Number(sec / 60)) + ':' + playerInstance.pad(Number(sec % 60));
    }
    else
    {
