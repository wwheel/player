...
import { IDisplayOptions } from './types/display-options';
...
export class Wwplayer
{
  ...
  constructor()
  {
    ...
    // noinspection JSUnresolvedVariable
    this.version     = typeof FP_BUILD_VERSION !== 'undefined' ? FP_BUILD_VERSION : '';
    // noinspection JSUnresolvedVariable
    this.homepage    = typeof FP_HOMEPAGE !== 'undefined'
      ? FP_HOMEPAGE + '/?utm_source=player&utm_medium=context_menu&utm_campaign=organic'
      : '';
    this.destructors = [];
  }

  init(playerTarget: HTMLVideoElement|string, options: IDisplayOptions): void
  {
    const self = this;
    ...
    playerNode.addEventListener('pause', this.onFluidPlayerPause);
    ...
    playerNode.play = function ()
    {
        ...
        if (typeof DeviceOrientationEvent !== 'undefined' && typeof (DeviceOrientationEvent as any).requestPermission === 'function')
        {
          (DeviceOrientationEvent as any).requestPermission()
            .then(function (response)
            {
              if (response === 'granted')
              {
                this.debugMessage('DeviceOrientationEvent permission granted!');
              }
            })
            .catch(console.error);
        }
      ...
      try
      {
        ...
        if (promise !== undefined && promise !== null)
        {
          ...
          }).catch(error =>
          {
            ...
            {
              this.announceLocalError(202, 'Failed to play video.');
            }
          ...
          {
            if (this.isPlayingMedia === false)
            {
              this.announceLocalError(204, '[FP_ERROR] Timeout error. Failed to play video?');
            }
          }, 5000);
      ...
      {
        console.error('[FP_ERROR] Playback error', error);
        this.announceLocalError(201, 'Failed to play video.');
      }
    };
    ...
    try
    {
      ...
      {
        for (const textTrack of this.domRef.player.textTracks)
        {
  ...
  announceLocalError(code: string, msg?: string): void
  {
  ...
  generateCustomControlTags(options: ICustomControlTagsOptions): ICustomControlTags
  {
    ...
    controls.root.id        = this.videoPlayerId + '_fluid_controls_container';
    ...
    controls.playPause.id        = this.videoPlayerId + '_fluid_control_playpause';
    ...
    if (options.controlForwardBackward)
    {
      ...
      controls.skipBack.id        = this.videoPlayerId + '_fluid_control_skip_back';
      ...
      controls.skipForward.id        = this.videoPlayerId + '_fluid_control_skip_forward';
    ...
    controls.progressContainer.id        = this.videoPlayerId + '_fluid_controls_progress_container';
    ...
    controls.fullscreen.id        = this.videoPlayerId + '_fluid_control_fullscreen';
    ...
    controls.theatre.id        = this.videoPlayerId + '_fluid_control_theatre';
    ...
    controls.cardboard.id        = this.videoPlayerId + '_fluid_control_cardboard';
    ...
    controls.subtitles.id        = this.videoPlayerId + '_fluid_control_subtitles';
    ...
    controls.videoSource.id        = this.videoPlayerId + '_fluid_control_video_source';
    ...
    controls.playbackRate.id        = this.videoPlayerId + '_fluid_control_playback_rate';
    ...
    controls.download.id        = this.videoPlayerId + '_fluid_control_download';
    ...
    controls.volumeContainer.id        = this.videoPlayerId + '_fluid_control_volume_container';
    ...
    controls.volume.id        = this.videoPlayerId + '_fluid_control_volume';
    ...
    controls.volumeCurrent.id        = this.videoPlayerId + '_fluid_control_currentvolume';
    ...
    controls.volumeCurrentPos.id        = this.videoPlayerId + '_fluid_control_volume_currentpos';
    ...
    controls.mute.id        = this.videoPlayerId + '_fluid_control_mute';
    ...
    controls.duration.id        = this.videoPlayerId + '_fluid_control_duration';
  ...
  {
    const playPauseButton = (this.domRef.player.parentNode as HTMLElement).getElementsByClassName('fluid_control_playpause');
    const menuOptionPlay  = document.getElementById(this.videoPlayerId + 'context_option_play');
    const controlsDisplay = (this.domRef.player.parentNode as HTMLElement).getElementsByClassName('fluid_controls_container');
    const fpLogo          = document.getElementById(this.videoPlayerId + '_logo');

    const initialPlay = document.getElementById(this.videoPlayerId + '_fluid_initial_play');
    if (initialPlay)
    {
      document.getElementById(this.videoPlayerId + '_fluid_initial_play').style.display        = 'none';
      document.getElementById(this.videoPlayerId + '_fluid_initial_play_button').style.opacity = '1';
    }
    ...
    {
      document.getElementById(this.videoPlayerId + '_fluid_initial_play').style.display        = 'block';
      document.getElementById(this.videoPlayerId + '_fluid_initial_play_button').style.opacity = '1';
    }
  ...
  playPauseAnimationToggle(play): void
  {
    ...
    if (play)
    {
      document.getElementById(this.videoPlayerId + '_fluid_state_button').classList
        .remove('fluid_initial_pause_button');
      document.getElementById(this.videoPlayerId + '_fluid_state_button').classList.add('fluid_initial_play_button');
    }
    else
    {
      document.getElementById(this.videoPlayerId + '_fluid_state_button').classList.remove('fluid_initial_play_button');
      document.getElementById(this.videoPlayerId + '_fluid_state_button').classList.add('fluid_initial_pause_button');
    }

    document.getElementById(this.videoPlayerId + '_fluid_initial_play').classList.add('transform-active');
    setTimeout(
      () =>
      {
        document.getElementById(this.videoPlayerId + '_fluid_initial_play').classList.remove('transform-active');
      },
      800
    );
  }
  ...
  {
    const currentProgressTag = (this.domRef.player.parentNode as HTMLElement).getElementsByClassName('fluid_controls_currentprogress');
  ...
  controlDurationUpdate(id?: any): void
  {
    ...
    const timePlaceholder = this.domRef.player.parentNode.getElementsByClassName('fluid_control_duration');
  ...
  {
    const currentVolumeTag    = document.getElementById(this.videoPlayerId + '_fluid_control_currentvolume');
    const volumeposTag        = document.getElementById(this.videoPlayerId + '_fluid_control_volume_currentpos');
    const volumebarTotalWidth = document.getElementById(this.videoPlayerId + '_fluid_control_volume').clientWidth;
    const volumeposTagWidth   = volumeposTag.clientWidth;
    const muteButtonTag       = (this.domRef.player.parentNode as HTMLElement).getElementsByClassName('fluid_control_mute');
    ...
    if (0 !== this.domRef.player.volume)
    {
      ...
      this.fluidStorage.fluidMute = false;
    }
    else
    {
      this.fluidStorage.fluidMute = true;
    }
  ...
  muteToggle(): void
  {
    ...
    // Persistent settings
    this.fluidStorage.fluidVolume = this.latestVolume;
    this.fluidStorage.fluidMute   = this.domRef.player.muted;
  }
  ...
  fullscreenToggle(): void
  {
    ...
    const fullscreenButton               = (videoPlayerTag.parentNode as HTMLElement).getElementsByClassName('fluid_control_fullscreen');
  ...
  getEventOffsetY(evt, el): number
  {
    ...
    const videoWrapper       = this.findClosestParent(el, 'div[id^="fluid_video_wrapper_"]');

    if (videoWrapper)
    {
      const videoPlayerId = videoWrapper.id.replace('fluid_video_wrapper_', '');

      const requestFullscreenFunctionNames = this.checkFullscreenSupport('fluid_video_wrapper_' + videoPlayerId);
  ...
  onProgressbarMouseDown(event): void
  {
    ...
    {
      initialPosition = this.getEventOffsetX(event, document.getElementById(this.videoPlayerId + '_fluid_controls_progress_container'));
    }
    ...
    {
      const totalWidth = document.getElementById(this.videoPlayerId + '_fluid_controls_progress_container').clientWidth;
      if (totalWidth)
      {
  ...
  initialPlay(): void
  {
    ...
    if (!this.displayOptions.layoutControls.playButtonShowing)
    {
      ...
      const initialControlsDisplay = document.getElementById(this.videoPlayerId + '_fluid_controls_container');
  ...
  playPauseToggle(): void
  {
    ...
    else
    {
      ...
      {
        this.domRef.player.src = 'https://cdn.fluidplayer.com/static/blank.mp4';
        this.domRef.player.play();
        this.playPauseAnimationToggle(true);
      }
    ...
    if (isFirstStart)
    {
      ...
      const pseudoPoster = document.getElementById(this.videoPlayerId + '_fluid_pseudo_poster');
      if (pseudoPoster)
      {
    ...
    const blockOnPause = document.getElementById(this.videoPlayerId + '_fluid_html_on_pause');
  ...
  setCustomControls(): void
  {
    ...
    this.trackEvent(this.domRef.player.parentNode, 'click', '.fluid_control_playpause', () =>
    {
      if (!this.firstPlayLaunched)
      {
        this.domRef.player.removeEventListener('play', this.initialPlay);
    ...
    this.domRef.player.addEventListener('fluidplayerpause', () =>
    {
      this.controlPlayPauseToggle();
    }, false);
    ...
    {
      document.getElementById(this.videoPlayerId + '_fluid_controls_progress_container')
        .addEventListener(eventOn, event => this.onProgressbarMouseDown(event), false);
    }

    //Set the volume controls
    document.getElementById(this.videoPlayerId + '_fluid_control_volume_container')
    ...
    this.trackEvent(this.domRef.player.parentNode, 'click', '.fluid_control_mute', () => this.muteToggle());

    this.setBuffering();

    //Set the fullscreen control
    this.trackEvent(this.domRef.player.parentNode, 'click', '.fluid_control_fullscreen', () => this.fullscreenToggle());

    // Theatre mode
    if (this.displayOptions.layoutControls.allowTheatre && !this.isInIframe)
    {
      document.getElementById(this.videoPlayerId + '_fluid_control_theatre').style.display = 'inline-block';
      this.trackEvent(this.domRef.player.parentNode, 'click', '.fluid_control_theatre', () => this.theatreToggle());
    }
    else
    {
      document.getElementById(this.videoPlayerId + '_fluid_control_theatre').style.display = 'none';
    }
  ...
  createTimePositionPreview(): void
  {
    ...
    const progressContainer = document.getElementById(this.videoPlayerId + '_fluid_controls_progress_container');
    const previewContainer  = document.createElement('div');

    previewContainer.id             = this.videoPlayerId + '_fluid_timeline_preview';
    ...
    document.getElementById(this.videoPlayerId + '_fluid_controls_progress_container')
      .addEventListener('mousemove', event =>
      {
        ...
        const hoverTimeItem     = document.getElementById(this.videoPlayerId + '_fluid_timeline_preview');
  ...
  setCustomContextMenu(): void
  {
    ...
    divContextMenu.id             = this.videoPlayerId + '_fluid_context_menu';
    ...
    menuItemVersion.innerHTML = 'Fluid Player ' + this.version;
  ...
  {
    this.domRef.wrapper.className += ' fluid_player_layout_' + this.displayOptions.layoutControls.layout;
  ...
  setupPlayerWrapper(): HTMLDivElement
  {
    ...
    wrapper.id        = 'fluid_video_wrapper_' + this.videoPlayerId;
  ...
  createVideoSourceSwitch(): void
  {
    ...
    sourceChangeList.id            = this.videoPlayerId + '_fluid_control_video_source_list';
    ...
    for (const source of this.videoSources)
    {
      ...
      sourceChangeDiv.addEventListener('click', (event) =>
      {
        ...
        this.domRef.player.style.width  = this.domRef.player.clientWidth + 'px';
        this.domRef.player.style.height = this.domRef.player.clientHeight + 'px';
        ...
        this.videoSources.forEach(source =>
        {
          ...
          {
            this.setBuffering();
            this.setVideoSource(source.url);
            this.fluidStorage.fluidQuality = source.title;
          }
        });
    ...
    else
    {
      ...
      document.getElementById(this.videoPlayerId + '_fluid_control_video_source').style.display = 'none';
    }
  }
  ...
  initHtmlOnPauseBlock(): void
  {
    ...
    containerDiv.id            = this.videoPlayerId + '_fluid_html_on_pause';
  ...
  initPlayButton(): void
  {
    ...
    const containerDiv     = document.createElement('div');
    containerDiv.id        = this.videoPlayerId + '_fluid_initial_play_button';
    containerDiv.className = 'fluid_html_on_pause';
    ...
    containerDiv.innerHTML = '<div id="' + this.videoPlayerId + '_fluid_initial_play" class="fluid_initial_play" style="background-color:' + backgroundColor + '"><div id="' + this.videoPlayerId + '_fluid_state_button" class="fluid_initial_play_button"></div></div>';
  ...
  hideControlBar(): void
  {
    ...
    const divVastControls = (this.domRef.player.parentNode as HTMLElement).getElementsByClassName('fluid_controls_container');
  ...
  setBuffering(): void
  {
    ...
    const bufferBar = (this.domRef.player.parentNode as HTMLElement).getElementsByClassName('fluid_controls_buffered');
  ...
  setPersistentSettings(): boolean
  {
    ...
    if (typeof (this.fluidStorage.fluidVolume) !== 'undefined'
    ...
    if (typeof (this.fluidStorage.fluidQuality) !== 'undefined'
